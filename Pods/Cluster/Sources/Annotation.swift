...
open class Annotation: MKPointAnnotation {
    ...
    open var locationData : [String : String]?
...
open class ClusterAnnotationView: MKAnnotationView {
    ...
    open func configure() {
        ...
        if mapType == "Review"{
            countLabel.text = "\(count) DJS"
        }else{
            countLabel.text = "\(count) projects"
        }
        
    }
}
open class DJMultipleCluserPin: MKAnnotationView {
    open lazy var countLabel: UILabel = {
        let label = UILabel()
        label.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        label.backgroundColor = .clear
...
open class SinglePinClusterView: MKAnnotationView {
    open lazy var priceLabel: UILabel = {
        let label = UILabel()
        label.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        label.backgroundColor = .clear
        label.font = .boldSystemFont(ofSize: 13)
...
open class StyledClusterAnnotationView: ClusterAnnotationView {
    ...
    open override func configure() {
        ...
        case let .image(image):
            ...
            //ashitesh - you can coment below only one line to check image change or not
            self.image = image
           // self.image = UIImage(named: "img3")
